...
// TestCompleteUpdateFlow_AutoStrategy 测试完整的自动更新流程
func TestCompleteUpdateFlow_AutoStrategy(t *testing.T) {
	...
	// 注意：我们不能在测试中实际执行 performUpdate，因为它会调用 Restart() 并退出进程
	// 但我们可以测试各个组件
...
// TestCompleteUpdateFlow_ManualStrategy 测试完整的手动更新流程
...
// TestUpdateFlow_SingBoxContinuesRunning 测试更新过程中 sing-box 继续运行
func TestUpdateFlow_SingBoxContinuesRunning(t *testing.T) {
	...
	// 模拟 sing-box 正在运行
	agent.singboxCmd = nil // 在测试中不实际启动
	
	// 验证 performUpdate 的逻辑
	// 注意：我们不能实际调用 performUpdate，因为它会尝试重启进程
...
// TestUpdateFlow_ErrorHandling 测试更新流程的错误处理
...
// TestUpdateFlow_CommandLineFlag 测试命令行参数
